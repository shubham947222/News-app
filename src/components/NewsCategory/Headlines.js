...
const Headlines = ({ news, setNews, Title, keyword }) => {
  console.log(Title);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://api.mediastack.com/v1/news?access_key=${API_KEY}&sources=cnn,bbc,ndtv&keywords=${keyword}&sort=published_desc`
      )
      .then((res) => setNews(res.data.data))
      .catch((err) => console.log(err.message));
  }, [Title, keyword, setNews]);
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-evenly" }}>
        ...
        >
          <span style={{ fontWeight: "bold" }}>{Title}</span> news on NewsFeeder
        </h1>
        <div
          style={{
            position: "relative",
        ...
        >
          <h5>Follow Us:</h5>
          <FacebookIcon style={{ color: "crimson" }} />
          <TwitterIcon style={{ color: "crimson" }} />
              ...
              >
                ...
                <div style={{ display: "flex" }}>
                  ...
                  >
                    ...
                    >{`Source: ${
                      news.source ? news.source : "Anonymous"
                    } `}</Card.Title>
                    <Card.Text>
                      {news.description
                        ? `${
                  ...
                  >
                    ...
                    >{`Date: ${news.published_at.slice(0, 10)}`}</Card.Text>
                  </Card.Body>
                </div>
              </Card>
            </Container>
